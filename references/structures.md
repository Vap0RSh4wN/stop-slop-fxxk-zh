...
| "这套更稳" / "会把它拦下来" / "冒出新的 AI 词" | The sentence evaluates the rule or uses metaphor instead of stating the coverage condition. | Name the mechanism directly: which slots or tests still apply when the wording changes, and what kind of sentence will therefore be judged low-information. |
| Formal note sentence written with colloquial verbs, metaphor, or evaluation | Spoken phrasing blurs the object and weakens the note's criteria. | Replace it with the literal object, operation, condition, or failure mode. |
